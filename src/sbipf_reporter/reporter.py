...
def format_as_csv(holdings: list[Holding], output_path: Path) -> None:
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["コード", "銘柄名", "口座", "買付日", "数量", "取得単価", "現在値", "評価額", "損益", "損益率"])
...
def format_as_markdown(holdings: list[Holding], output_path: Path) -> None:
    ...
    lines = ["# SBI証券ポートフォリオ", "", "| コード | 銘柄名 | 口座 | 買付日 | 数量 | 取得単価 | 現在値 | 評価額 | 損益 | 損益率 |", "|------|------|------|------|------|------|------|------|------|------|"]

    for h in holdings:
        profit_rate = (h.profit_loss / h.evaluation_value * 100) if h.evaluation_value != 0 else 0.0
        lines.append(f"| {h.code or '-'} | {h.name} | {h.account_type.value} | {h.buy_date} | {h.quantity:,} | ¥{h.average_price:,.0f} | ¥{h.current_price:,.0f} | ¥{h.evaluation_value:,.0f} | ¥{h.profit_loss:+,.0f} | {profit_rate:+.2f}% |")
    ...
    lines.extend(["", f"**合計**: 保有数 {len(holdings)}件, 総資産 ¥{total_eval:,.0f}, 損益 ¥{total_profit:+,.0f} ({total_rate:+.2f}%)"])
